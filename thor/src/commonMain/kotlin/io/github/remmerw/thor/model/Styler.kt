...
object Styler {
    fun parseTextIntent(value: String): TextIndent? = runCatching {
        parseTextUnit(value).let { size ->
            if (size == null) {
                debug("parse text intent fail: $value")
                null
            } else {
    ...
    fun parseTextDecoration(value: String): TextDecoration? = runCatching {
        val list = buildList {
        ...
        if (list.isEmpty()) {
            debug("parse text decoration fail: $value")
            null
        } else {
    ...
    fun parseTextAlign(value: String): TextAlign? = when (value) {
        ...
        "justify", "justify-all" -> TextAlign.Justify
            ...
            debug("parse text align fail: $value")
            null
        }
    }

    fun parseFontWeight(value: String): FontWeight? = runCatching {
        when (value) {
            ...
            "bold" -> FontWeight.Bold
                ...
                if (int != null) {
                    ...
                    debug("parse text weight fail: $value")
                    null
                }
            }
        }
    ...
    fun parseFontStyle(value: String): FontStyle? =
        when (value) {
            ...
            "italic" -> FontStyle.Italic
                ...
                debug("parse font style fail: $value")
                null
            }
        }

    fun parseBackgroundColor(cssColor: String): Color? = if (cssColor == "transparent") {
        Color.Transparent
    } else {
        parseColor(cssColor).also { color ->
            if (color == null) {
                debug("unsupported parse background color: $cssColor")
            }
        }
    }

    fun parseFontSize(value: String): TextUnit? = runCatching {
        parseTextUnit(value).also {
            if (it == null) {
                debug("unsupported parse font size$value")
            }
        }
    }.onFailure { e ->
        debug(e)
